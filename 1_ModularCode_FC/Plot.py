...
with open("C:/Users/Admin/PycharmProjects/ModularCode_FC/LeNet_MNIST_history_main_Experiment.pkl", "rb") as f:
...
#Percentages
        ...
        for row in range(len(history_main)):
                ...
                x3.append((history_main[row][3]['val_accuracy']))
                x31.extend((history_main[row][3]['val_accuracy']))
                x4.append((history_main[row][5]['val_accuracy']))
                x41.extend((history_main[row][5]['val_accuracy']))
                x5.append((history_main[row][7]['val_accuracy']))
                x51.extend((history_main[row][7]['val_accuracy']))
                x6.append((history_main[row][8]['val_accuracy']))
                x61.extend((history_main[row][8]['val_accuracy']))
                x7.append((history_main[row][10]['val_accuracy']))
                x71.extend((history_main[row][10]['val_accuracy']))
        ...
        plt.plot((pd.DataFrame(x2).mean(axis = 0)).index.values, pd.DataFrame(x2).mean(axis = 0), label=str(np.around(history_main[1][1]['percentage_wts_remaining'], decimals=3)), color = 'orange')
        plt.fill_between((pd.DataFrame(x2).mean(axis = 0)).index.values, pd.DataFrame(x2).mean(axis = 0)-x21, pd.DataFrame(x2).mean(axis = 0)+x21, alpha=.2, color = 'orange')

        plt.plot((pd.DataFrame(x3).mean(axis = 0)).index.values, pd.DataFrame(x3).mean(axis = 0), label=str(np.around(history_main[1][3]['percentage_wts_remaining'], decimals=3)) , color = 'green')
        plt.fill_between((pd.DataFrame(x3).mean(axis = 0)).index.values, pd.DataFrame(x3).mean(axis = 0)-x31, pd.DataFrame(x3).mean(axis = 0)+x31, alpha=.2, color = 'green')

        plt.plot((pd.DataFrame(x4).mean(axis = 0)).index.values, pd.DataFrame(x4).mean(axis = 0), label=str(np.around(history_main[1][5]['percentage_wts_remaining'], decimals=3)), color = 'yellow')
        plt.fill_between((pd.DataFrame(x4).mean(axis = 0)).index.values, pd.DataFrame(x4).mean(axis = 0)-x41, pd.DataFrame(x4).mean(axis = 0)+x41, alpha=.2, color = 'yellow')

        plt.plot((pd.DataFrame(x5).mean(axis = 0)).index.values, pd.DataFrame(x5).mean(axis = 0), label=str(np.around(history_main[1][7]['percentage_wts_remaining'], decimals=3)), color = 'red')
        plt.fill_between((pd.DataFrame(x5).mean(axis = 0)).index.values, pd.DataFrame(x5).mean(axis = 0)-x51, pd.DataFrame(x5).mean(axis = 0)+x51, alpha=.2, color = 'red')

        plt.plot((pd.DataFrame(x6).mean(axis = 0)).index.values, pd.DataFrame(x6).mean(axis = 0), label=str(np.around(history_main[1][8]['percentage_wts_remaining'], decimals=3)), color = 'pink')
        plt.fill_between((pd.DataFrame(x6).mean(axis = 0)).index.values, pd.DataFrame(x6).mean(axis = 0)-x61, pd.DataFrame(x6).mean(axis = 0)+x61, alpha=.2, color = 'pink')

        plt.plot((pd.DataFrame(x7).mean(axis = 0)).index.values, pd.DataFrame(x7).mean(axis = 0), label=str(np.around(history_main[1][10]['percentage_wts_remaining'], decimals=3)), color = 'cyan')
        plt.fill_between((pd.DataFrame(x7).mean(axis = 0)).index.values, pd.DataFrame(x7).mean(axis = 0)-x71, pd.DataFrame(x7).mean(axis = 0)+x71, alpha=.2, color = 'cyan')
        ...
        plt.plot((pd.DataFrame(x3).mean(axis = 0)).index.values, pd.DataFrame(x3).mean(axis = 0), label=str(np.around(history_main[1][2]['percentage_wts_remaining'], decimals=3)) , color = 'green')
        plt.fill_between((pd.DataFrame(x3).mean(axis = 0)).index.values, pd.DataFrame(x3).mean(axis = 0)-x31, pd.DataFrame(x3).mean(axis = 0)+x31, alpha=.2, color = 'green')

        plt.plot((pd.DataFrame(x4).mean(axis = 0)).index.values, pd.DataFrame(x4).mean(axis = 0), label=str(np.around(history_main[1][3]['percentage_wts_remaining'], decimals=3)), color = 'yellow')
        plt.fill_between((pd.DataFrame(x4).mean(axis = 0)).index.values, pd.DataFrame(x4).mean(axis = 0)-x41, pd.DataFrame(x4).mean(axis = 0)+x41, alpha=.2, color = 'yellow')

        plt.plot((pd.DataFrame(x5).mean(axis = 0)).index.values, pd.DataFrame(x5).mean(axis = 0), label=str(np.around(history_main[1][4]['percentage_wts_remaining'], decimals=3)), color = 'red')
        plt.fill_between((pd.DataFrame(x5).mean(axis = 0)).index.values, pd.DataFrame(x5).mean(axis = 0)-x51, pd.DataFrame(x5).mean(axis = 0)+x51, alpha=.2, color = 'red')

        plt.plot((pd.DataFrame(x6).mean(axis = 0)).index.values, pd.DataFrame(x6).mean(axis = 0), label=str(np.around(history_main[1][5]['percentage_wts_remaining'], decimals=3)), color = 'pink')
        plt.fill_between((pd.DataFrame(x6).mean(axis = 0)).index.values, pd.DataFrame(x6).mean(axis = 0)-x61, pd.DataFrame(x6).mean(axis = 0)+x61, alpha=.2, color = 'pink')

        plt.plot((pd.DataFrame(x7).mean(axis = 0)).index.values, pd.DataFrame(x7).mean(axis = 0), label=str(np.around(history_main[1][6]['percentage_wts_remaining'], decimals=3)), color = 'cyan')
        plt.fill_between((pd.DataFrame(x7).mean(axis = 0)).index.values, pd.DataFrame(x7).mean(axis = 0)-x71, pd.DataFrame(x7).mean(axis = 0)+x71, alpha=.2, color = 'cyan')
