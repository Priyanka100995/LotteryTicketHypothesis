...
orig_model = Conv6Model.pruned_nn.pruned_nn(pruning_params_unpruned, pruning_params_unpruned, pruning_params_unpruned)
...
# Save random weights-
orig_model.save_weights("Conv6_CIFAR_Random_Weights.h5", overwrite=True)

# Save random weights-
orig_model.save_weights("Conv6_CIFAR_Winning_Ticket.h5", overwrite=True)
...
# Instantiate a model
model_gt = Conv6Model.pruned_nn.pruned_nn(pruning_params_unpruned, pruning_params_unpruned, pruning_params_unpruned)

mask_model = Conv6Model.pruned_nn.pruned_nn(pruning_params_unpruned, pruning_params_unpruned, pruning_params_unpruned)
...
for j in range (1, num_trials+1):
    ...
    for i in range(1, num_pruning_rounds + 1):
        ...
        model_gt.load_weights("Conv6_CIFAR_Winning_Ticket.h5")
        ...
        if i == 1:  # (For every 1st round of every trial set to initial wts)
            ...
            # Load winning ticket (from above)-
            model_gt.load_weights("Conv6_CIFAR_Random_Weights.h5")
        ...
        model_gt.save_weights("Conv6_CIFAR_Trained_Weights.h5", overwrite=True)
        ...
        pruning_params_conv = {
            'pruning_schedule': Prune.ConstantSparsity(
                target_sparsity=PruningPercent.conv1_pruning[i - 1], begin_step=1000,
                end_step=Dataset.Data.end_step, frequency=100
            )
        }
        ...
        pruned_model = Conv6Model.pruned_nn.pruned_nn(pruning_params_conv, pruning_params_fc, pruning_params_op)

        # Load weights from original trained and unpruned model-
        pruned_model.load_weights("Conv6_CIFAR_Trained_Weights.h5")
        ...
        history_pruned = pruned_model.fit(
            ...
            epochs=num_epochs,
            verbose=1,
            callbacks=Conv6Model.pruned_nn.callback,
        ...
        pruned_model.save_weights("Conv6_CIFAR_Pruned_Weights.h5", overwrite=True)
        ...
        mask_model.load_weights("Conv6_CIFAR_Pruned_Weights.h5")
        ...
        winning_ticket_model = Conv6Model.pruned_nn.pruned_nn(pruning_params_unpruned, pruning_params_unpruned, pruning_params_unpruned)

        # Load weights of PRUNED model-
        winning_ticket_model.load_weights("Conv6_CIFAR_Pruned_Weights.h5")
        ...
        winning_ticket_model.save_weights("Conv6_CIFAR_Magnitude_Based_Winning_Ticket_Distribution_{0}.h5".format(
...
print("\nIterative-pruning for VGG Conv6 NN using Lottery Ticket Hypothesis & Magnitude-based weight pruning is now complete.\n")
...
with open("C:/Users/Admin/PycharmProjects/ModularCode_Conv6/Conv6_CIFAR10.pkl", "wb") as f:
