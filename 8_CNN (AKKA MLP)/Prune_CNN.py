...
import os
import yaml
from CNN_0_9_0 import log_dir_model1
from m_architecture_CNN_0_9_1 import r_squared
...
train_samples = pd.read_csv(data_path + "train_samples.csv", delimiter=',')
train_targets = pd.read_csv(data_path + "train_targets.csv", delimiter=',')
val_samples = pd.read_csv(data_path + "val_samples.csv", delimiter=',')
val_targets = pd.read_csv(data_path + "val_targets.csv", delimiter=',')
test_samples = pd.read_csv(data_path + "test_samples.csv", delimiter=',')
test_targets = pd.read_csv(data_path + "test_targets.csv", delimiter=',')  # TODO: Änderung kurzfristig
...
val_samples = val_samples.reshape(val_samples.shape[0], val_samples.shape[1], 1)
...
print("Validation ground truth shape: ", val_targets.shape)
print("Test ground truth shape: ", test_targets.shape)
...
# Save random weights-
orig_model.save_weights("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Weights/Model_Random_Weights.h5", overwrite=True)
...
total_params = conv1 + conv2 + dense1 + op_layer
...
while loc_tot_params >= max_pruned_params:
    ...
    loc_tot_params = + loc_conv1 + loc_conv2 + loc_dense1 + loc_op_layer
...
print("Convolutional Layer Pruning Percent", conv1_pruning)
...
def prune_model(model, pruning_params_conv, pruning_params_fc, pruning_params_op):
    '''
...
orig_model_pruned = prune_model(orig_model, conv1_pruning[0], dense1_pruning[0], op_layer_pruning[0])
...
for j in range (1, num_trials+1):
    ...
    for i in range(1, num_pruning_rounds+1):
        ...
        history['r_squared'] = np.zeros(shape=num_epochs)

        history['val_r_squared'] = np.zeros(shape=num_epochs)
        history['train_loss'] = np.zeros(shape=num_epochs)
    ...
    for i in range(1, num_pruning_rounds + 1):
        ...
        if i == 1:  # (For every 1st round of every trial set to initial wts)
            ...
            # Load winning ticket (from above)-
            model_gt.load_weights("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Weights/Model_Random_Weights.h5")
        ...
        winning_ticket_model.save_weights("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Weights/Model_Magnitude_Based_Winning_Ticket_Distribution_{0}.h5".format(
            history_main[i]['percentage_wts_pruned']), overwrite=True)
        winning_ticket_model.save("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Pruned_Models/Magnitude_Based_Pruned_Model_{0}.h5".format(
            history_main[i]['percentage_wts_pruned']), overwrite=True)
...
print("Iterative pruning of Convolutional Neural Network Complete! ")

import os
import pickle

os.getcwd()
with open("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Pruned_Models_history_main_Experiment.pkl", "wb") as f:
    pickle.dump(History_data, f)

with open("C:/Users/Priyanka-Sanjeev.BHO/PycharmProjects/CNN/Pruned_Models_history_main_Experiment.pkl", "rb") as f:
...
def plot_history(self, history):
        ...
        plt.xlabel('Epochs')
        plt.title('Loss eval')
        plt.savefig("Loss evaluation.png")
        #plt.show()

plot_history(history_main)